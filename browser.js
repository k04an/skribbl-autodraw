...
const main = async () => {
    ...
    try {
        ...
        await page.goto(config.privateRoom ? config.privateRoom : 'https://skribbl.io/')
    } catch (e) {
    ...
    browserEmitter.on('startDraw', async (array) => {
        ...
        // Рисуем каждый пиксель изображения
        array.forEach(row => {
            row.forEach((pixel, index) => {
                // Проверяем совпадает ли цвет текущего пикселя с цветом предыдущего, для проведения линии
                if (index != 0 && config.optimise) {
                    if (pixel.row == row[index - 1].row && pixel.num == row[index - 1].num) {
                        if (index == row.length - 1) {
                            page.evaluate((context) => {
                                let moveEvent = new PointerEvent('pointermove', {
                                    clientX: 540 + context.x,
                                    clientY: 200 + context.y,
                                    pointerType: 'mouse',
                                })
                                document.dispatchEvent(moveEvent)
                                document.dispatchEvent(new PointerEvent('pointerup', { pointerType: 'mouse' }))
                            }, {x: xOffset, y: yOffset})
                        }
                        xOffset += 5
                        return
                ...
                page.evaluate((context) => {
                    ...
                    document.dispatchEvent(moveEvent)
                    document.dispatchEvent(new PointerEvent('pointerup', { pointerType: 'mouse' }))
                    document.querySelector(`.colors > ${context.row} > div:nth-child(${context.num})`).dispatchEvent(colorEvent)
            ...
            // Перед отрисовкой новой строки выполняем 3 события указателя, двигаем, нажимаем и отпускаем
            // Это нужно чтобы избежать отрисовки кривых линий между строками
            page.evaluate((context) => {
